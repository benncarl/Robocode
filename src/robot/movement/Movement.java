...
public class Movement {
	...
	private final static double WALL_STRENGTH = 50000;
	private final static double TARGET_STRENGTH = -10000;
	...
	public void updateGravPoints(List<Enemy> enemies, double myX, double myY) {
		...
		for(Enemy enemy : enemies) {
			if(target && enemy.getLastDistance() > 200) {
				this.gravPoints.add(new GravityPoint(enemy.getPoint(), TARGET_STRENGTH, false));
				target = false;
			} else {
